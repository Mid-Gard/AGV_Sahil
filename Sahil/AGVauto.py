...
def get_current_position():
    while True:
        # Get the current location and yaw from the vehicle
        lat = vehicle.location.global_frame.lat
        lon = vehicle.location.global_frame.lon
        yaw_raw = math.degrees(vehicle.attitude.yaw)
        yaw = (yaw_raw + 360) % 360
        return lat, lon, yaw
...
def navigate_to_waypoint(current_position, target_position):
    ...
    while distance > 1.0:  # Adjust threshold as needed
        ...
        # Calculate the deviation angle using the yaw
        deviation_angle = bearing - yaw
        deviation_angle = (deviation_angle + 360) % 360
        if deviation_angle > 180:
            deviation_angle -= 360

        # Calculate deviation from the path
        deviation_distance = math.sin(math.radians(deviation_angle)) * distance
